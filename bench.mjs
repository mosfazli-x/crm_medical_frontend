...
const SAMPLES = [
  { id: 's0', path: 'C:\\Users\\Cursed Saint\\AppData\\Local\\Temp\\opencode\\ocr-bench\\s0.png', gt: 'Sara Mohammadi' },
  { id: 's1', path: 'C:\\Users\\Cursed Saint\\AppData\\Local\\Temp\\opencode\\ocr-bench\\s1.png', gt: '09123456789' },
  { id: 's2', path: 'C:\\Users\\Cursed Saint\\AppData\\Local\\Temp\\opencode\\ocr-bench\\s2.png', gt: 'Mr. Brown commented icily.' },
  { id: 's3', path: 'C:\\Users\\Cursed Saint\\AppData\\Local\\Temp\\opencode\\ocr-bench\\s3.png', gt: 'Tehran Valiasr Street 123' },
  { id: 's4', path: 'C:\\Users\\Cursed Saint\\AppData\\Local\\Temp\\opencode\\ocr-bench\\s4.png', gt: 'Zahra Hosseini' },
  { id: 's5', path: 'C:\\Users\\Cursed Saint\\AppData\\Local\\Temp\\opencode\\ocr-bench\\s5.png', gt: 'Kermanshah Imam Khomeini St' },
  { id: 's7', path: 'C:\\Users\\Cursed Saint\\AppData\\Local\\Temp\\opencode\\ocr-bench\\s7.png', gt: 'Reza Ahmadi' },
]

const modelId = process.env.MODEL_ID
const dtype = process.env.DTYPE || 'q8'
const numBeams = parseInt(process.env.NUM_BEAMS || '1', 10)
const maxTokens = parseInt(process.env.MAX_TOKENS || '96', 10)
...
function wordScore(pred, gt) {
  ...
  return hit / Math.max(g.length, 1)
}
...
for (const s of SAMPLES) {
  const t1 = Date.now()
  const out = await classifier(s.path, gen)
  ...
  rows.push({
    ...
    pred: text,
    gt: s.gt,
    ws: wordScore(text, s.gt),
    cs: charScore(text, s.gt),
    ms,
  })
}
