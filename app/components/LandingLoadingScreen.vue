<template>
  <Transition name="loader-exit" @after-leave="$emit('finished')">
    <div
      ...
      :class="isLight ? 'bg-slate-50' : 'bg-[#060a13]'"
      role="status"
      aria-live="polite"
    ...
    >
      ...
      <div class="absolute inset-0 overflow-hidden">
        <div class="loader-orb loader-orb--1 absolute w-[600px] h-[600px] rounded-full blur-[120px] -top-40 -right-32" :class="isLight ? 'bg-blue-400/15' : 'bg-blue-500/20'"></div>
        <div class="loader-orb loader-orb--2 absolute w-[500px] h-[500px] rounded-full blur-[100px] -bottom-36 -left-28" :class="isLight ? 'bg-cyan-300/10' : 'bg-cyan-400/15'"></div>
        <div class="loader-orb loader-orb--3 absolute w-[400px] h-[400px] rounded-full blur-[90px] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" :class="isLight ? 'bg-indigo-300/8' : 'bg-indigo-400/10'"></div>
      </div>

      <!-- Grid pattern overlay -->
      <div class="absolute inset-0" :style="{ opacity: isLight ? 0.03 : 0.05, backgroundImage: 'linear-gradient(rgba(59,130,246,0.5) 1px, transparent 1px), linear-gradient(90deg, rgba(59,130,246,0.5) 1px, transparent 1px)', backgroundSize: '60px 60px' }"></div>
      ...
      <div class="relative z-10 flex flex-col items-center gap-8">
        ...
        <div class="loader-3d-container perspective-[1000px]">
          <div class="loader-3d-element relative w-32 h-32">
            <!-- Front face -->
            <div class="loader-face loader-face--front absolute inset-0 flex items-center justify-center rounded-2xl bg-gradient-to-br from-blue-500 to-cyan-400 shadow-2xl shadow-blue-500/30">
              <img src="~/assets/images/hastihoseinilogo.png" alt="" class="w-16 h-16 object-contain" width="48" height="48" />
            </div>
            <!-- Back face -->
            <div class="loader-face loader-face--back absolute inset-0 flex items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-400 to-blue-500 shadow-2xl shadow-cyan-400/30">
        ...
        <div class="loader-text-container text-center">
          <h1 class="loader-logo-text text-4xl font-black tracking-tight bg-gradient-to-r from-blue-400 via-cyan-400 to-blue-500 bg-clip-text text-transparent" style="background-size: 200% auto;">
            کلینیک تخصصی دکتر حسینی
          </h1>
        ...
        <div class="loader-progress-container relative w-16 h-16">
          ...
          <div class="absolute inset-0 flex items-center justify-center">
            <span class="text-xs font-bold tabular-nums" :class="isLight ? 'text-blue-600' : 'text-cyan-400'">
        ...
        <div class="loader-status-container h-5">
          <Transition name="status-fade" mode="out-in">
            <span :key="statusIndex" class="text-xs font-medium" :class="isLight ? 'text-slate-500' : 'text-slate-400'">
      ...
      <div class="absolute inset-0 pointer-events-none overflow-hidden">
        <div v-for="i in 20" :key="i" class="loader-particle absolute w-1 h-1 rounded-full" :class="isLight ? 'bg-cyan-400/20' : 'bg-blue-400/30'" :style="particleStyle(i)"></div>
      </div>
    </div>
  </Transition>
</template>
...
.loader-orb--2 { animation-duration: 22s; animation-delay: -4s; }
.loader-orb--3 { animation-duration: 25s; animation-delay: -8s; }
...
@keyframes loader-rotate3d {
  ...
  25% { transform: rotateY(90deg) rotateX(15deg) scale(1.05); }
  50% { transform: rotateY(180deg) rotateX(0deg) scale(1); }
  75% { transform: rotateY(270deg) rotateX(-15deg) scale(1.05); }
}
