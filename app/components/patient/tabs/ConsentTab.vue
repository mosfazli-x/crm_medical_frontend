<template>
  ...
  <div v-else>
    <div class="flex items-center justify-between mb-4">
      ...
      <v-btn variant="tonal" color="black" size="small" prepend-icon="mdi-plus" class="rounded-lg" @click="emit('create')">ثبت رضایت‌نامه جدید</v-btn>
    </div>
    ...
    <div v-else class="space-y-3">
      <div v-for="c in records" :key="c.id" class="bg-white border border-slate-200 rounded-lg p-4 flex items-center justify-between">
        <div class="flex-1">
          ...
          <p class="text-xs text-slate-500 mt-1">تاریخ: {{ formatDate(c.date || c.created_at) }}</p>
