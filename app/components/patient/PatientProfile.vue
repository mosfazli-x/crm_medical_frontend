      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <div class="flex items-center gap-3 mt-1 text-sm text-gray-500">
            ...
            <span>{{ $t('patientProfile.insurance') }}: <span class="font-medium text-gray-700">{{ patient.insurance_code || $t('patientProfile.noInsurance') }}</span></span>
          </div>
        </div>
      </div>
    ...
    <div class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      ...
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <span class="block text-xs text-gray-500 mb-1">{{ $t('patientProfile.phone') }}</span>
            <span class="text-sm font-medium text-gray-900">{{ patient.phone || '---' }}</span>
          </div>
          <div>
            <span class="block text-xs text-gray-500 mb-1">{{ $t('patientProfile.birthDate') }}</span>
            <span class="text-sm font-medium text-gray-900">{{ patient.birth_date || '---' }}</span>
          </div>
          <div class="col-span-2">
            <span class="block text-xs text-gray-500 mb-1">{{ $t('patientProfile.address') }}</span>
      ...
      <div class="space-y-4">
        ...
        <div>
          <span class="block text-xs text-gray-500 mb-2">{{ $t('patientProfile.allergies') }}</span>
          ...
          <span v-else class="text-sm text-gray-400">{{ $t('patientProfile.none') }}</span>
        </div>

        <div>
          <span class="block text-xs text-gray-500 mb-2">{{ $t('patientProfile.diseases') }}</span>
      ...
      <div class="space-y-4">
        <h3 class="text-sm font-bold text-teal-500 uppercase tracking-wider flex items-center gap-2 border-b pb-2">
          {{ $t('patientProfile.currentRecord') }}
        </h3>
        
        <div>
          <span class="block text-xs text-gray-500 mb-1">{{ $t('patientProfile.visitNotes') }}</span>
          <p class="text-sm text-gray-700 bg-gray-50 p-3 rounded-lg border border-gray-100 min-h-[80px] whitespace-pre-wrap">
            {{ patient.latest_visit?.pregnancy_notes || $t('patientProfile.noNotes') }}
          </p>
        </div>
      </div>
